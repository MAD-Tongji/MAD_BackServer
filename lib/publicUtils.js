...
/**
 * 要上传的空间: bucket = 'madtest';
 */

/**
 * 七牛云生成upload token
 * 构建上传策略函数
 * 使用时传入bucket参数, 示例: var uptoken = uptoken('madtest');
 */
...
function getToken(userid){
    ...
    ref.child('token2id').orderByChild('id').equalTo(userid).once('value',(snap)=>{
        ...
        {
            ref.child('token2id').child(key).remove((err)=>{
                if(err==null){
                    ref.child('token2id').update(newMapping);
                }else{
                    console.log(redLog('更新token2id失败！严重错误！'));
                }
            });
        }
        if(snap.val()==null){
...
function fakeMemcache(){
    ref.child('token2id').limitToFirst(999999999).on('value',(snap)=>{
        Token2Id_Memcached = snap.val();
        // console.log(Token2Id_Memcached);
    });
    ref.child('advertisment').limitToFirst(999999999).on('value',(snap)=>{
...
function playAd(token,adid){
    ...
    statisticRef.child('/admin/homePageData').limitToFirst(999999999).once('value',(snap)=>{
        if(snap.val()==null){
    ...
    ref.child('user/'+usrid).once('value',(snap)=>{
        if(snap.val()!=null){
            ref.child('user/'+usrid+'/adUsedList').push(adid);
            var adprice = getAdDetail(adid).price;
            var balance = snap.val().balance + adprice;
            console.log(balance);
            ref.child('user/'+userid+'/balance').update(balance);
        }
    });
