...
var statisticRef = ref.child('statistic/user');
...
function playAd(token,adid){
    ...
    var userStatisticReference =  statisticRef.child(usrid);
    userStatisticReference.once('value',(snap)=>{
        ...
        else
        {
            ...
            userStatisticReference.update({totalIncome:newIncome},(err)=>{
                if(err==null)
                {
                    console.log(greenLog('succeded'));
                }
...
function monthlyStatistic(monthlyRef,ADID){
    ...
    statisticRef.child(thisMonth).once('value',(snap)=>{
        if(snap.val() == null)
        {
            ...
            newMonth[thisMonth].top5[ADID]={
                income:adprice,
                title:adtitle
            }
            statisticRef.child(thisMonth).update(newMonth,(err)=>{
                if(err == null)
                {
